module MoonshineRuby
  mattr_accessor :ruby_interpreter

  def ruby(interpreter = :debian, uri = "")
    recipe :moonshine_ruby, { :interpreter => interpreter.to_sym, :uri => uri }
    send(:include, InstanceMethods)
  end

  module InstanceMethods
    def moonshine_ruby(options)
      self.class.ruby_interpreter = options[:interpreter]
      case options[:interpreter]
      when :enterprise
        # tarball = options[:uri].split("/").last
        ...
        #   :command  => "/usr/bin/wget #{options[:uri]}",
